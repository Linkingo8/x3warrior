...
CallbackReturn WheelBalancingController::on_activate(const rclcpp_lifecycle::State &)
{
    ...
    LK_L_handles_  = get_LK_state(wheel_joint_name_.at(0));
    LK_R_handles_ =  get_LK_state(wheel_joint_name_.at(1));
    if (!imu_handles_) {
        return CallbackReturn::ERROR;
    }
    if (!LK_L_handles_) {
...
std::shared_ptr<LK9025Handle> WheelBalancingController::get_LK_state(const std::string & joint_name)
{
    // Lookup the pitch state interface
